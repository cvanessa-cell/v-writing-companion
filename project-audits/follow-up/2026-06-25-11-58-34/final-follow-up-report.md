...
- `npm run test`: passed after fixes.
- `npm run build`: passed after fixes, including desktop renderer and extension bundle.
...
- Broadened root validation so root `build` and `lint` cover shared, desktop, and extension surfaces.
...
- `apps/desktop/src/main/aiProvider.ts`
- `apps/desktop/src/preload/index.ts`
- `apps/desktop/src/renderer/components/MemoryCenter.tsx`
- `apps/desktop/src/renderer/components/RewritePanel.tsx`
...
- Desktop output built under `apps/desktop/out`.
- Browser extension bundle built under `apps/browser-extension/dist`.
...
- The repo still contains unrelated uncommitted work in `apps/desktop/src/main/index.ts` outside this automation.
...
- Unrelated work left untouched: `apps/desktop/src/main/index.ts`, `.assistant-runs/`, `docs/performance-audit-2026-06-25.md`, and the local recovery folder.

## 23. Deployment status

- Detectable deploy target: local Windows NSIS packaging config in [`apps/desktop/electron-builder.json`](C:/Users/cvane/V/apps/desktop/electron-builder.json).
...
| ---- | ----------- | -------- | ----------- | ------------- | ------ | -------------- |
| Core functionality | Root scripts missed shipped surfaces and the renderer build hid a JSX blocker | Reproducible root build/lint failures | Extended root build/lint coverage and normalized `RewritePanel.tsx` | `package.json`, `packages/shared/package.json`, `apps/browser-extension/package.json`, `apps/browser-extension/tsconfig.json`, `apps/desktop/src/renderer/components/RewritePanel.tsx` | Repo is shippable from the root command surface | Add bridge/integration tests |
| Reliability/performance | Extension settings polling ran on inactive tabs | Audit evidence in `contentScript.ts` | Visibility-aware polling with focus/visibility refresh | `apps/browser-extension/src/contentScript.ts` | Lower idle overhead and less tab-wide churn | Narrow injection scope and add telemetry |
| Admin/privacy | Exclusions were stored but not fully reviewable/removable | Memory service returned exclusions but UI hid them | Added exclusion tabs, guidance, and delete path in Memory Center | `apps/desktop/src/renderer/components/MemoryCenter.tsx` | Stronger trust and supportability | Add richer diagnostics/status badges |
...
| Quality gate | Desktop TypeScript issues blocked reliable linting | `npm run lint` failures in `aiProvider.ts` and preload cleanup typing | Added explicit types and safe cleanup return | `apps/desktop/src/main/aiProvider.ts`, `apps/desktop/src/preload/index.ts` | Clean root lint | Keep root checks green as features expand |
